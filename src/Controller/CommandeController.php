...
#[Route('/{id}/annuler_commande', name: 'commande_client_annuler', methods: ['POST'])]
public function annulerfront(Request $request, Commande $commande, EntityManagerInterface $entityManager): Response
{
    // Check if the user confirmed the action (CSRF protection)
    if ($this->isCsrfTokenValid('annuler'.$commande->getId(), $request->request->get('_token'))) {
        // Set the status to "Annulée"
    ...
    // Redirect back to the list or the edit page
    return $this->redirectToRoute('app_commande_front');
}
